...
void mouse_callback(int event, int x, int y, int flag, void *param) {
  ...
  if (event == cv::EVENT_LBUTTONDOWN) {
    ...
    if (points_selected > 1 && points_selected <= 4) {
      ...
      cv::line(source_image, source_points[points_selected - 2],
               source_points[points_selected - 1], green, 2, 8, 0);
      cv::imshow("selection", source_image);
      cv::moveWindow("selection", 0, 0);
    }
  ...
  // mouse pointer tracking
    ...
    if (points_selected > 0 && points_selected < 4) {
      ...
      cv::line(cache_image, source_points[points_selected - 1], newpt, green, 2,
               8, 0);
